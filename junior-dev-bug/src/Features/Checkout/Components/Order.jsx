...
export default function Order() {
    const { open, setTotalPrice, totalPrice,toggleModal } = useGlobalCtx();
            ...
            <div className="border border-border border-opacity-5 rounded-[0.5rem] py-4 px-5">
                ...
                <table className="w-full">
                    <tbody>
                        ...
                        {CartProducts.map((product) => <TbRow key={product.id} id={product.id} label={product.product}>৳ {product.price} TK </TbRow>)}
                ...
                <Btn onClick={()=>{toggleModal()}}>Continue to Payment</Btn>
                {open ? <PaymentMd /> : ""}
            </div>
        </div >
    )
